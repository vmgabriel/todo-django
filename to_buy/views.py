...
class DetailToBuyView(LoginRequiredMixin, generic.detail.DetailView):
    ...
    comment_model = models.ItemListComment

    def queryset_list(self, pk: int) -> QuerySet:
        return models.ItemListToBuy.objects.filter(enabled=True, list=pk)

    def queryset_products(self, pk: int) -> QuerySet:
        my_list: list[int] = [
            x.product.pk
            for x in self.queryset_list(pk)
        ]
        data = (
    ...
    def get_comments(self, pk) -> list:
        comments = self.comment_model.objects.filter(enabled=True, list=pk)
        return comments
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        context['items'] = items
        context["comments"] = self.get_comments(context["listtobuy"])
...
class ToBuyNewCommentView(LoginRequiredMixin, generic.edit.FormView):
    model = models.ItemListComment
    template_name = "to_buy/comment/edit.html"
    form_class = forms.ItemListCommentForm
    success_url = 'to_buy:buys_add_product'
    mode = "Save"
    ...
    def form_valid(self, form):
        ...
        self.object.save()
        return redirect(self.get_success_url(), pk=self.kwargs["list_pk"])
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        context["mode"] = self.mode
        context["list_pk"] = self.kwargs["list_pk"]
        return context
...
@login_required
def delete_comment_to_list(request, list_pk, pk):
    model = models.ItemListComment
    comment = get_object_or_404(model, pk=pk)
    comment.enabled = False
    comment.save()
    return redirect("to_buy:buys_add_product", pk=list_pk)
...
def convert_comment(data: models.ItemListComment):
    return """
    {user}: {comment}
    """.format(user=data.created_by.username, comment=data.comment)


def convert_list_to_message(pk, dataquery: list[QuerySet[models.ListToBuy]]) -> str:
    data = list(map(convert_item, dataquery))
    query_comments = models.ItemListComment.objects.filter(enabled=True, list=pk)
    comments = list(map(convert_comment, query_comments))
    return """
    List to Buy
    ----
    {}
...
def telegram_send_list(request, pk):
    ...
    telegram.send_message(
        convert_list_to_message(pk, items),
        "+573125086310"
    )
    return redirect("to_buy:buys_show", pk=pk)
